...
export function DeploymentHistoryItem({ deployment, onDelete }: { deployment: DeployedWorkflow, onDelete: (id: string) => void }) {
    ...
    return (
        <AccordionItem value={deployment?.id} className="border-white/[0.05] bg-white/[0.02] rounded-xl px-4 mb-2.5">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2 bg-black/40 p-2 rounded-lg border border-white/5">
                            ...
                            <button onClick={copyUrl} className="text-neutral-500 hover:text-white shrink-0">
                                {copied ? <Check size={10} /> : <Copy size={10} />}
                            </button>
                        </div>
                    </div>
                    <div className="space-y-1">
                        <span className="text-[10px] text-neutral-400 uppercase font-black">Created At</span>
                        <p className="text-[11px] text-neutral-300 pt-2 font-mono">
                            {new Date(deployment?.createdAt).toLocaleString()}
                        </p>
                    </div>
                </div>
                <div className="bg-neutral-900/50 p-3 rounded-lg border border-white/[0.03]">
                    <span className="text-[10px] text-neutral-500 uppercase font-black block mb-2">Metadata</span>
                    <div className="flex gap-4">
                        <div className="text-[11px] text-neutral-400 font-mono">Nodes: {JSON.stringify(deployment?.nodes)?.length} bytes</div>
                        <div className="text-[11px] text-neutral-400 font-mono">Edges: {JSON.stringify(deployment?.edges)?.length} bytes</div>
                    </div>
                </div>
            </AccordionContent>
        </AccordionItem>
