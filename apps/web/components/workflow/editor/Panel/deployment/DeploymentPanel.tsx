...
import {Rocket, History, Settings2, EyeOff, Copy, AlertTriangle, Activity, Eye} from "lucide-react";
...
export function DeploymentPanel({
    ...
    const generateKey = () => {
        const key = `nrn_live_${Math.random().toString(36).substring(7)}`;
        setSecretKey(key);
        setShowKey(true);
    };
    ...
    return (
        ...
        >
            <Tabs defaultValue="deploy" className="w-full flex flex-col h-full">
                <TabsList className="bg-white/[0.02] border border-white/[0.05] p-1 h-12 self-start ml-4 mb-6">
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-7 no-scrollbar!">
