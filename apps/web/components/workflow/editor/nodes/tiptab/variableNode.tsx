  ...
  renderHTML({ HTMLAttributes }) {
    return [
      'span',
      {
        'data-variable': '',
        class:
          'px-1.5 py-0.5 rounded bg-neutral-800 text-primary border border-neutral-700 font-medium mx-0.5',
      },
      `{{${HTMLAttributes.id}}}`,
    ];
  },
...
export const VariableSuggestion = (
  variables: Variable[]
): Omit<SuggestionOptions, 'editor' | 'command'> => ({
  char: '{',
  allowSpaces: true,
  startOfLine: false,
  ...
  render: () => {
    ...
    return {
      onStart: (props) => {
        ...
        component.className =
          'bg-neutral-900 border border-neutral-700 rounded-md text-xs py-1 shadow-xl min-w-[120px] z-[9999] relative';

        props.items.forEach((item) => {
          const option = document.createElement('div');
          option.className =
            'px-3 py-2 cursor-pointer hover:bg-neutral-800 text-neutral-300';
