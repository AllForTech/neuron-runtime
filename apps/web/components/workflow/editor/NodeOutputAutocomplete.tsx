...
export const NodeOutputAutocomplete = ({
  onSelect,
}: NodeOutputAutocompleteProps) => {
    ...
    for (const key in fields) {
      ...
      if (fields[key].type === 'object') {
        results = results.concat(
          renderFields(fields[key].children || {}, path)
        );
      } else if (
        fields[key].type === 'array' &&
        fields[key].arrayItem?.type === 'object'
      ) {
        results = results.concat(
          renderFields(fields[key].arrayItem.children || {}, path + '[]')
        );
      } else {
        results.push(path);
      }
