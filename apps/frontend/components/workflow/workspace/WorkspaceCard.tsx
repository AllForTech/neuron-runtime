    ...
    return (
        ...
        >
            <div className="flex flex-col h-full">
                {/* 1. The 2x2 Mini Preview Grid */}
                <div className="grid grid-cols-2 grid-rows-2 gap-2 p-3 flex-grow">
                    {workflowList.slice(0, 4).map((wf) => (
                        <div
                            key={wf.id}
                            className={cn(
                                "group/wf relative flex flex-col justify-between overflow-hidden rounded-xl p-3 transition-all duration-300 cursor-pointer",
                                "bg-muted/40 border border-white/[0.03] hover:bg-neutral-900 hover:border-white/10 hover:shadow-lg"
                            )}
                            onClick={(e) => {
                                e.stopPropagation(); // Don't trigger the "Open Workspace" footer
                        ...
                        >
                            ...
                            {/* Inner Actions */}
                            <div className="absolute bottom-2 right-1 opacity-0 group-hover/wf:opacity-100 transition-opacity">
                    ...
                    {Array.from({ length: Math.max(0, 4 - workflowCount) }).map((_, i) => (
                        <div key={`empty-${i}`} className="rounded-xl border border-dashed border-white/[0.03] opacity-40" />
                    ))}
                </div>

                {/* 2. Workspace Navigation Footer */}
                <div
