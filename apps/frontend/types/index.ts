...
export type RuntimeAction =
    | { type: RuntimeActionType.SET_NODE_STATUS; nodeId: string; status: ExecutionStatus }
    | { type: RuntimeActionType.SET_EDGE_ACTIVE; edgeId: string; isActive: boolean }
    | { type: RuntimeActionType.SET_EXECUTIONS; payload: Record<string, Execution> }
    | { type: RuntimeActionType.ADD_EXECUTION; execution: Execution }
    | { type: RuntimeActionType.UPDATE_EXECUTION; executionId: string; payload: Partial<Execution> }
    | { type: RuntimeActionType.ADD_LOG; payload: ExecutionLog }
    | { type: RuntimeActionType.SET_LOGS; payload: Record<string, ExecutionLog> }
    | { type: RuntimeActionType.UPDATE_LOGS; id: string; payload: Partial<ExecutionLog> }
