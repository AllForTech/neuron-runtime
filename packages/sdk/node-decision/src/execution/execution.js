...
const executor = async ({ config, }) => {
    const { input, rules, inputTransforms = [] } = config;
    const runTransform = (val, list) => {
        let res = val;
        if (list.includes("toString"))
            res = String(res ?? "");
        if (list.includes("toLowerCase"))
            res = String(res).toLowerCase();
        if (list.includes("toUpperCase"))
            res = String(res).toUpperCase();
        if (list.includes("trim"))
            res = String(res).trim();
        if (list.includes("toNumber"))
            res = Number(res);
        return res;
    };
